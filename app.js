(function () {
  ...
  var DAYS = window.A3_DAYS || {};
  var MANIFEST = window.A3_MANIFEST || [];
  var PLAN = window.A3_PLAN || [];
  ...
  function cacheBust(src) {
    var cb = window.__A3_CB || new Date().toISOString().slice(0, 10).replace(/-/g, "");
  ...
  function scoreKey(d) { return "a3_score_" + d; }
  function refKey(d) { return "a3_reflection_" + d; }
  function examAnsKey(d) { return "a3_examans_" + d; }
  ...
  // ---------- 30 天学习计划（来源：A3学习型管理-近30天日历.md，纯展示计划 + 叠加实际进度） ----------
  ...
  function renderPlan() {
    ...
    var html = '<div class="plan-head">' +
      ...
      '<div class="plan-sub">第 1 天 = 2026-08-13' + (todayDay ? ' · 今日 = Day ' + todayDay : '') + ' · 已学 ' +
  ...
  function renderResources(d) {
    ...
    var html = '<p class="ref-hint">以下资源均经检索核对、链接真实可查，延伸理解 A3 的起源与丰田实践：</p>';
  ...
  function copyAndOpenBoard() {
    ...
    var line = "【A3学习上报】姓名：" + name + (dept ? "｜部门：" + dept : "") +
  ...
  function exportArchive() {
    var data = { type: "a3-archive", version: 1, at: Date.now(), days: {} };
    MANIFEST.forEach(function (m) {
      var d = m.date;
    ...
    a.href = url;
    a.download = "A3学习档案_" + new Date().toISOString().slice(0, 10) + ".json";
    a.click();
  ...
  function importArchive(file) {
    ...
    reader.onload = function () {
      try {
        ...
        renderSidebar(); updateProgress(); renderCalendar(); renderRecent30(); renderPlan();
  ...
  //  A) 归档格式 {"days":{...}}  B) {"a3_score_...":"...",...}（copy(JSON.stringify(localStorage)) 的输出）
  //  C) 多行 "a3_xxx: 值" 或 "a3_xxx\t值"（DevTools 逐行复制）
    ...
    try {
      ...
      if (obj && obj.days) {
        ...
        Object.keys(obj).forEach(function (k) {
          if (k.indexOf("a3_") === 0) { setIfAbsentRaw(k, obj[k]); imported++; }
        });
      }
    } catch (e) {
      text.split(/\r?\n/).forEach(function (ln) {
        var m = ln.match(/^(a3_[A-Za-z0-9_]+)\s*[:=]\s*(.*)$/) || ln.match(/^(a3_[A-Za-z0-9_]+)\t(.*)$/);
        if (m) { setIfAbsentRaw(m[1], m[2]); imported++; }
      });
    }
    if (!imported) { $("importMsg").textContent = "没识别到 a3_ 开头的数据，请确认粘贴内容来自 DevTools 的 localStorage"; return; }
    renderSidebar(); updateProgress(); renderCalendar(); renderRecent30(); renderPlan();
    if (currentDate) { renderExam(DAYS[currentDate]); renderReflection(currentDate); }
  ...
  var AUTO_BACKUP_NAME = "A3学习档案-自动备份.json";
  ...
  function scheduleAutoBackup() {
    try { localStorage.setItem("a3_lastbackup_at", String(Date.now())); } catch (e) {}
